...
import { Connection, ScramCredentials, ScramSha256, ScramSha512 } from '../../../src/client/session';
import { PlainSaslMechanism, SaslAuthenticator } from '../../../src/client/session/security/authenticators/sasl';
    ...
    describe('SCRAM-SHA-256', () => {
      beforeAll(async () => {
        container = await new KafkaContainer('confluentinc/cp-kafka:7.3.2')
          ...
          .withSaslSslListener({
            keystore: {
              content: readFileSync(join(__filename, '../../../certs/kafka.keystore.jks')),
              passphrase: 'changeit'
            },
            sasl: {
    ...
    describe('PLAIN', () => {
      beforeAll(async () => {
        container = await new GenericContainer('confluentinc/cp-kafka:latest')
          ...
          .withBindMounts([
            {
              source: join(__filename, '../../../../kafka_server_jaas.conf'),
