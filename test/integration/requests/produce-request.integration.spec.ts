...
describe('ProduceRequest', () => {
  ...
  beforeAll(async () => {
    container = await new KafkaContainer('confluentinc/cp-kafka:latest').withExposedPorts(port).withReuse().start();
  ...
  describe('v3', () => {
    it.each(allAcks)('should send the request and properly parse the response for acks: %d', async (acks) => {
      ...
      const response = await connection.send(request, produceRequestBuilder.expectResponse());
